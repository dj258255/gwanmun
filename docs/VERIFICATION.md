...
로드맵의 "실측 필수" 원칙에 따라, 각 단계가 실제로 도는 증거를 남긴다. 지어낸 수치 없음.

---

## 2단계 — TCP 프레이밍 · 프로토콜 변환
...
## 3단계 — API 게이트웨이 층(인증·라우팅·유량제어) + 모듈러 모놀리스

2단계까지는 통로(전문↔JSON 왕복)만이었다. 3단계는 그 통로 앞에 문지기를 세운다 —
...
인증→라우팅→유량제어를 다 통과한 뒤에야 2단계의 전문 왕복이 실행된다(문지기 통과 후 통역).
...
## 4단계 — 가변길이 전문(길이 헤더) · 커넥션 풀

2단계는 "고정 61byte" 전문만 프레이밍했고, 커넥션 풀과 길이 헤더(가변 전문)를 잔여로 남겼다.
4단계가 그 둘을 채운다 — 레코드가 건수만큼 붙어 길이가 매번 다른 거래내역 조회 전문을 4byte ASCII
...
## 5단계 — 관측 가능한 거래 원장 (거래ID 채번 · 3값 상태 · 마스킹 · 메트릭)

4단계까지의 게이트웨이는 거래를 흘려보내기만 하고 아무것도 기억하지 못했다. 5단계는 모든 거래에
...
- **JWT/OAuth 여전히 미구현**(3단계 잔여 그대로), 원장 보존 기한·파티셔닝·감사 추적(변경 이력) 없음.

---

## 6단계 — 장애 내성(서킷브레이커·재시도·데드라인) · UNKNOWN 해소(거래상태조회·망취소)

5단계는 응답을 못 받은 거래를 UNKNOWN으로 **기록**까지만 했다. 6단계는 두 가지를 채운다 —
...
- **`ddl-auto: update`는 체크 제약을 갱신하지 않는다.** 상태 enum에 CANCELED를 추가하자, 5단계
...
## 7단계 — 감사 결함 소탕(풀 고갈 3중 오작동·원장 공백·유휴 TTL·재시드·fail-closed 인코딩) + 보안 경화
...
- **A3 — 서킷 stale 결과 귀속**: OPEN 전이 직전에 나가 있던 호출의 늦은 onSuccess/onFailure가 새
  ...
  [ROADMAP 감사 백로그](ROADMAP.md)에 기록. → **8단계에서 수정.**
- 나머지 백로그(CI·k6 부하 실측·멱등키·EOD 대사 배치·Boot 3.5+)는 ROADMAP 감사 백로그 절 참조.

---

## 8단계 — CI · A3 서킷 stale 귀속 수정 · k6 부하 실측 · Boot 3.5 업그레이드

7단계가 A3(서킷 stale 결과 귀속)를 "난이도 대비 실익이 낮다"며 백로그로 미뤘다. 8단계는 부하
...
## 9단계 — 멱등키 · EOD 대사 배치 · DBTower 연계

8단계까지 남은 두 구멍을 채운다 — (A) 호출자 재전송을 게이트웨이가 구분해 **이중 거래 0**을
